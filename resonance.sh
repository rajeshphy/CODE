...
if [[ -s "$toc_txt" ]]; then
    ...
    # Create markdown with bold titles and red-colored page numbers using raw LaTeX
    {
        echo "# Contents"
        echo
        awk '{
            match($0, /[0-9]{3,4}-[0-9]{3,4}$/)
            if (RSTART > 0) {
    ...
    # Convert to PDF using pandoc and pdflatex with spacing, font, color
    pandoc "$toc_md" \
        --pdf-engine=pdflatex \
        ...
        -H <(echo '\usepackage{setspace}\usepackage{xcolor}\doublespacing\vspace*{2\baselineskip}') \
        -o "$toc_pdf"
fi
